...
compare_node_by_addr(const void **a, const void **b)
{
  const node_t *n1 = *a;
  const node_t *n2 = *b;
  if (n1->rs->addr < n2->rs->addr) return -1;
  else if (n1->rs->addr > n2->rs->addr) return 1;
  else return 0;
}
...
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit_ei, int flags)
{
  ...
  if (purpose != CIRCUIT_PURPOSE_C_GENERAL || or_options->ORPort_set || !tightrope_enabled ||
    ...
    circuit_shadow_global_circuit_list_lock();
    ...
    circuit_shadow_global_circuit_list_unlock();
